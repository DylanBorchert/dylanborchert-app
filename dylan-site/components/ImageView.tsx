...
export default function ImageView({ item, index }: any) {
    ...
    return (
        <div key={index} className='max-w-[1060px] mx-auto px-5 py-5'>
            ...
            {show && <div className='bg-black fixed w-full h-[100dvh] z-50 top-0 left-0 animate-fade-down-nav'>
                ...
                <Image src={`https://strapi.phantommedia.online${item.image.data.attributes.url}`} width={1000} height={1000} alt='' key={index} className='h-[100dvh] w-auto m-auto' />
            </div>}
        </div>
    )
}